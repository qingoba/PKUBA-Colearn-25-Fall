...
1. 以太坊允许在链上部署和运行智能合约。ERC20 是一种代币合约接口标准，规定了代币应具备的基本函数和事件（如 transfer、Transfer 事件）。USDC、USDT 等并不是以太坊协议内置的币，而是部署在以太坊主网上、遵循 ERC20 标准的智能合约实例。每一种 ERC20 代币都对应一个独立的合约地址，因此一条链上可以同时存在大量不同的代币。

2. 程序首先通过 rpc.Dial 建立与 Infura 以太坊节点的 RPC 通道，再用 ethclient 对 RPC 进行以太坊语义封装，形成统一的客户端入口。
